...
namespace ndn::chunks
{
  ...
  PipelineInterestsAdaptive::sendInterest(uint64_t segNo, bool isRetransmission)
  {
    ...
    if (!segInfo.isfirst)
    {
      segInfo.total_delay += time::steady_clock::now() - segInfo.timeSent;
    }
    segInfo.timeSent = time::steady_clock::now();
    segInfo.isfirst = false;
  ...
  PipelineInterestsAdaptive::wait()
  {
    ...
    {
      spdlog::debug("pipeline has sent all the segments and can schedule next packet");
  ...
  PipelineInterestsAdaptive::handleData(const Interest &interest, const Data &data)
  {
    ...
    if (segInfo.total_delay == time::nanoseconds(0))
    {
      segInfo.total_delay = rtt;
    }
    if (m_options.isVerbose)
    {
      std::cerr << "Received segment #" << recvSegNo
                << ", rtt=" << rtt.count() / 1e6 << "ms"
                << ", rto=" << segInfo.rto.count() / 1e6 << "ms" << ", total_delay=" << segInfo.total_delay.count() / 1e6 << "ms\n";
