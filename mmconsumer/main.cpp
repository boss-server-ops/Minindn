#include "chunk/consumer.hpp" // 添加Consumer类头文件
...
namespace ndn::chunks
{
    ...
    main(int argc, char *argv[])
    {
        ...
        try
        {
            // 创建单个Face
            Face face;
            spdlog::info("Created single Face for consumer");

            // 创建RTT估计器
            auto rttEstimator = std::make_unique<RttEstimatorWithStats>(std::move(rttEstOptions));
            ...
            auto discover = std::make_unique<DiscoverVersion>(face, Name(prefix).append("1"), options);
            ...
            //     {
            //         spdlog::error("Error while printing RTT estimator parameters: {}", e.what());
            //     }
            // }
            ...
            auto consumer = std::make_unique<Consumer>(security::getAcceptAllValidator());

            spdlog::info("Starting consumer with prefix: {}", Name(prefix).append("1").toUri());

            consumer->run(std::move(discover), std::move(chunks));

            spdlog::info("Processing events on Face");
            face.processEvents();
