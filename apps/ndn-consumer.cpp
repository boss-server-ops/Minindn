...
void Consumer::ConstructAggregationTree()
{
    ...
    else if (m_topologyType == "LINE")
    {

        filename = "../topologies/LineTopology.txt";
        spdlog::info("Line topology is chosen.");
    }
    else
    {
...
void Consumer::OnData(const ndn::Interest &interest, const ndn::Data &data)
{
    ...
    if (sumParameters.find(seq) == sumParameters.end())
    {
        ...
        if (partialAggResult.size() >= m_dataQueue)
        {
            ...
            // zyx: schedule next packet drectly and let pcon handle this situation
            m_scheduleEvent[name_sec0] = m_scheduler.schedule(ndn::time::microseconds(0),
                                                              [this, name_sec0]
