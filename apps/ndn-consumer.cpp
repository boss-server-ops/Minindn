...
void Consumer::SendInterest(std::shared_ptr<ndn::Name> newName)
{
    ...
    // Start response time
    rttStartTime[nameWithSeq] = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
...
void Consumer::RTORecorder(std::string prefix)
{
    ...
    auto now = std::chrono::steady_clock::now();
    file << std::chrono::duration_cast<std::chrono::microseconds>(now - startTime).count() << " " << RTO_threshold[prefix].count() << std::endl;
...
void Consumer::InitializeParameter()
{
    ...
    for (const auto &round : globalTreeRound)
    {
        ...
        for (const auto &prefix : round)
        {
            ...
            NS_LOG_DEBUG("Init rate limit - " << m_rateLimit[prefix] * 1000 << " pkgs/ms.");
        }
        i++;
    }
