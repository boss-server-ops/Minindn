...
//     spdlog::debug("Flow {} - name -> {}: timeout.", name_sec0, interest.getName());
...
void Aggregator::StartApplication()
{
    ...
    // TODO:check if the addroute is necessary and maybe be implemented in minindn
...
//         {
//             NS_LOG_DEBUG("Flow - " << key << ". Interest queue size: " << interestQueue[key].size());
...
void Aggregator::OnInterest(const ndn::InterestFilter &filter, const ndn::Interest &interest)
{
    ...
    if (interestType == "data")
    {
        ...
        for (const auto &[key, value] : aggregationMap)
        {
            ...
            {
                isQueueFull = true;
                spdlog::debug("Interest queue of flow {} is full, drop it - {}", key, interest.getName().toUri());
                interestOverflow++;
        ...
        if (m_agg_newDataName.find(seq) != m_agg_newDataName.end() || map_agg_oldSeq_newName.find(seq) != map_agg_oldSeq_newName.end())
        {
            ...
            spdlog::debug("This is a retransmission interest from downstream, drop it - {}", interest.getName().toUri());
            downstreamRetxCount++;
            return;
        }
...
//         spdlog::info("Flow {} -> Schedule next sending event after {} ms.", prefix, nextTime / 1000);
//         // TODO: waiting for modification
...
//         spdlog::info("Flow {} -> Interest queue is empty. Schedule next sending event after {} ms.", prefix, nextTime / 1000);
//         m_scheduleEvent[prefix] = Simulator::Schedule(MicroSeconds(nextTime), &Aggregator::ScheduleNextPacket, this, prefix);
//     }
// }
...
//  */
// bool Aggregator::InterestSplitting(uint32_t seq)
// {
//     // Divide interests and push them into queue
//     for (const auto &[key, value] : aggregationMap)
//     {
...
//     NS_LOG_DEBUG("1: " << std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count() << " us");
...
//                     //! For testing purpose
//                     stop = std::chrono::high_resolution_clock::now();
//                     NS_LOG_DEBUG("2: " << std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count() << " us");
//                 }
//                 else
//                 {
...
//             //! For testing purpose
//             stop = std::chrono::high_resolution_clock::now();
//             NS_LOG_DEBUG("3: " << std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count() << " us");
...
// std::map<std::string, std::vector<std::string>> Aggregator::aggTreeProcessStrings(const std::vector<std::string> &inputs)
// {
//     // 占位实现
