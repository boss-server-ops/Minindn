...
ConsumerPcon::ConsumerPcon()
{
    ...
    ECNLocal = true;
    SetWindow(1);
    spdlog::info("ConsumerPcon initialized");
}

void ConsumerPcon::SendInterest(std::shared_ptr<ndn::Name> newName)
{
...
void ConsumerPcon::ScheduleNextPacket(std::string prefix)
{
...
void ConsumerPcon::StartApplication()
{
    Consumer::StartApplication();
}

void ConsumerPcon::OnData(const ndn::Interest &interest, const ndn::Data &data)
{
    spdlog::info("ConsumerPcon received data");
    ...
    spdlog::debug("Flow {} -> Window: {}, InFlight: {}", name_sec0, m_window[name_sec0], m_inFlight[name_sec0]);
    ...
    if (type == "data")
    {
        ...
        if (roundIndex == -1)
        {
            spdlog::error("Error on ronudIndex!");
            std::exit(EXIT_FAILURE);
        }
        ...
        // Congestion control
        // Priority
        // 1. Whether broadcasting("initialization") has finished - disabled
        // 2. PCON's ECN mark - disabled
        ...
        else if (data.getCongestionMark())
        {
            // Whether CWA is enabled
            // question: when does the m_useCwa change?
            if (m_useCwa && !CanDecreaseWindow(name_sec0, RTT_historical_estimation[name_sec0]))
            {
            ...
            else
            {
                ...
                WindowDecrease(name_sec0, "ConsumerCongestion");
                ...
                if (!throughputStable)
                {
                    ...
                    startThroughputMeasurement = now;
                }
...
void ConsumerPcon::SetWindow(uint32_t window)
{
    m_initialWindow = window;
    // m_window = m_initialWindow;
}

uint32_t ConsumerPcon::GetWindow() const
{
    return m_initialWindow;
}

void ConsumerPcon::WindowIncrease(std::string prefix)
{
...
void ConsumerPcon::WindowDecrease(std::string prefix, std::string type)
{
    ...
    else if (m_ccAlgorithm == CcAlgorithm::CUBIC)
    {
        ...
        {
            spdlog::error("Unknown congestion type, please check!");
            std::exit(EXIT_FAILURE);
        }
    }
    else
...
//    + */
void ConsumerPcon::CubicIncerase(std::string prefix)
{
...
void ConsumerPcon::CubicDecrease(std::string prefix, std::string type)
{
...
void ConsumerPcon::WindowRecorder(std::string prefix)
{
...
void ConsumerPcon::ResponseTimeRecorder(std::string prefix, bool flag)
{
...
void ConsumerPcon::InitializeLogFile()
{
 ...
 */
void ConsumerPcon::InitializeParameter()
{
...
void ConsumerPcon::OnNack(const ndn::Interest &interest, const ndn::lp::Nack &nack)
{
    Consumer::OnNack(interest, nack);
    spdlog::info("ConsumerPcon received nack");
}

void ConsumerPcon::OnTimeout(const ndn::Interest &interest)
{
    std::string prefix = interest.getName().get(0).toUri();

    WindowDecrease(prefix, "timeout");

    if (m_inFlight[prefix] > static_cast<uint32_t>(0))
    {
        m_inFlight[prefix]--;
    }

    spdlog::debug("Flow {} -> Window: {}, InFlight: {}", prefix, m_window[prefix], m_inFlight[prefix]);
    Consumer::OnTimeout(interest);
    spdlog::info("ConsumerPcon received timeout");
}

int main()
{
    ConsumerPcon consumerPcon;
    consumerPcon.StartApplication();
    return 0;
}