...
namespace ndn::chunks
{
    ...
    void Aggregator::chunkSendInterest()
    {
        spdlog::debug("Aggregator::chunkSendInterest()");

        m_childFaceThread = std::thread([this]()
                                        {
            try {
                spdlog::debug("child face thread start");
                m_childFace->processEvents();
                spdlog::debug("child face thread end");
            } 
            catch (const std::exception &e) {
                spdlog::error("child face error", e.what());
            } });
    }
